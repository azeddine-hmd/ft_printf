...
/*   Updated: 2019/12/30 16:16:02 by ahamdaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	xpre2int_maker(int iter, int *pre2int, char *s, va_list arglst)
{
	char	*precision;
...
char		*xargstr_maker(int iter, char *s, va_list arglst, char *flag)
{
	...
	if ((is_precisionexist = xpre2int_maker(iter, &precision2int, s, arglst))
			== -1)
		return (NULL);
	if (is_precisionexist == 0)
		return (ft_dec2hex(va_arg(arglst, unsigned)));
	argstr = NULL;
	argument = ft_dec2hex(va_arg(arglst, unsigned));
