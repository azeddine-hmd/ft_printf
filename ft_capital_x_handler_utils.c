...
static int	cxpre2int_maker(int iter, int *pre2int, char *s, va_list arglst)
{
	char	*precision;
...
char		*cxargstr_maker(int iter, char *s, va_list arglst, char *flag)
{
	...
	if ((is_precisionexist = cxpre2int_maker(iter, &precision2int, s, arglst))
			== -1)
		return (NULL);
	if (is_precisionexist == 0)
		return (ft_strtoupper(ft_dec2hex(va_arg(arglst, unsigned))));
	argstr = NULL;
	argument = ft_strtoupper(ft_dec2hex(va_arg(arglst, unsigned)));
